...
Describe 'Module Tests' -Tag Unit {
    Context "Module Tests" {
        ...
        It 'root module AzureServiceBusManagement.psm1 should exist' {
            $PathToModule | Should -Exist
            $? | Should -BeTrue
        } #psm1Exists
        It 'manifest should contain AzureServiceBusManagement.psm1' {
            $PathToManifest |
            Should -FileContentMatchExactly "AzureServiceBusManagement.psm1"
        } #validPSM1
